...
//!
//! Resolves cherry-pick conflicts for the specified paths by keeping the
//! "mine" (current branch) version. Only valid during an in-progress
...
pub struct CherryPickResolveMineArgs {
    /// Repository-relative paths to resolve in favor of "mine" (current branch).
...
/// Resolve cherry-pick conflicts by keeping the "mine" (current branch) version.
///
